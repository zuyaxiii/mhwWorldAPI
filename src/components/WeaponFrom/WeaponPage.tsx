...
const WeaponPage: React.FC = () => {
  ...
  const elements = [
    ...new Set(
      weaponData.flatMap((weapon) =>
        weapon.elements.map((element) => element.type)
      )
    ),
  ];

  const filteredweapon = weaponData.filter(
    (weapon) =>
      weapon.name.toLowerCase().includes(searchTerm.toLowerCase()) &&
      (selectedRarity === 0 - 9
        ? weapon.rarity >= 0 && weapon.rarity <= 9
        : true) && // เพิ่มเงื่อนไขสำหรับ "0-9"
      (selectedRarity === undefined
        ? true
        : weapon.rarity === Number(selectedRarity)) && // เงื่อนไขสำหรับ All Rarity
      (selectedTypes ? weapon.type === selectedTypes : true) &&
      (selectedElements.length > 0
        ? weapon.elements.some((element) =>
  ...
  return (
    <div>
      <SearchBar
        ...
        onRarityChange={(e) =>
          setSelectedRarity(e.target.value ? Number(e.target.value) : undefined)
        }
        rarity={rarity}
